...
#[command]
pub async fn init_translation_files(path: String, lang: String, translation_link: String) -> Result<(), String> {
    let base_path = Path::new(&path);
    ...
    // Utiliser reqwest asynchrone
    let response = reqwest::get(&translation_link)
        .await
...
#[command]
pub async fn is_translation_up_to_date(path: String, translation_link: String, lang: String) -> bool {
    let base_path = Path::new(&path);
    ...
    // Utiliser reqwest asynchrone
    let response = match reqwest::get(&translation_link).await {
        Ok(resp) => resp,
        Err(_) => return false,
    };
...
#[command]
pub async fn update_translation(path: String, lang: String, translation_link: String) -> Result<(), String> {
    let base_path = Path::new(&path);
