...
use scripts::updater_functions::{launch_updater, run_migration};
...
use scripts::third_party_apps::{launch_third_party_application, kill_third_party_application};
use std::sync::Mutex;
...
/// Récupère le statut des serveurs RSI en temps réel via l'API cState officielle
/// (https://status.robertsspaceindustries.com/index.json). Retourne un JSON
/// structuré avec le statut global + le statut individuel de chaque composant
/// (Platform, Persistent Universe, Arena Commander, etc.) et leurs incidents
/// non résolus.
#[command]
async fn fetch_rsi_status_feed() -> Result<String, String> {
    use serde_json::json;

    let client = reqwest::Client::new();
    let resp = client
        .get("https://status.robertsspaceindustries.com/index.json")
        .header("User-Agent", "StarTradFR/4.1.x")
    ...
    if let Some(systems) = v.get("systems").and_then(|x| x.as_array()) {
        for s in systems {
            ...
            if let Some(arr) = s.get("unresolvedIssues").and_then(|x| x.as_array()) {
                for iss in arr {
                    let title = iss.get("title").and_then(|x| x.as_str()).unwrap_or("");
                    let severity = iss.get("severity").and_then(|x| x.as_str()).unwrap_or("");
                    let permalink = iss.get("permalink").and_then(|x| x.as_str()).unwrap_or("");
                    issues.push(json!({
                        "title": title,
                        "severity": severity,
                        "permalink": permalink,
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            toggle_overlay_hub,
            is_overlay_hub_open,
            set_overlay_hub_mode,
            get_overlay_hub_mode,
            is_overlay_open,
            open_overlay,
