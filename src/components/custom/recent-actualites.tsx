...
const RSS_CACHE_KEY = 'startradfr_rss_actualites';

type RssCached = { items: RssItem[]; rawXml: string };

function getCachedRss(): RssCached | null {
    try {
        const cached = localStorage.getItem(RSS_CACHE_KEY);
        if (cached) return JSON.parse(cached) as RssCached;
    } catch {}
    return null;
}

type RssState = { items: RssItem[] | null; rawXml: string | null };

export default function RecentActualites({ max = 3 }: { max?: number }) {
    const cached = getCachedRss();
    const [{ items, rawXml }, setRssState] = useState<RssState>(
        cached ? { items: cached.items, rawXml: cached.rawXml } : { items: null, rawXml: null }
    );
    ...
    useEffect(() => {
        const fetchRss = async () => {
            ...
            try {
                xml_text = await fetchRssData();
                const parser = new XMLParser({
                    ignoreAttributes: false,
                    attributeNamePrefix: "@_"
                });
                const xml = parser.parse(xml_text);
                if (xml.feed?.entry) {
                    entries = Array.isArray(xml.feed.entry) ? xml.feed.entry : [xml.feed.entry];
                }
                try { localStorage.setItem(RSS_CACHE_KEY, JSON.stringify({ items: entries, rawXml: xml_text })); } catch {}
            } catch (err) {
                console.error('RSS fetch error:', err);
            }
            setRssState({ rawXml: xml_text, items: entries });
        };
        fetchRss();
    }, []);
