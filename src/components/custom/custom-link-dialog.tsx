...
type DialogState = { name: string; url: string; selectedIcon: string; showDeleteConfirm: boolean; iconPopoverOpen: boolean };
type DialogAction =
    | { type: 'SET_FIELD'; field: 'name' | 'url' | 'selectedIcon'; value: string }
    | { type: 'SET_DELETE_CONFIRM'; value: boolean }
    | { type: 'SET_ICON_POPOVER'; value: boolean }
    | { type: 'RESET'; editingLink?: CustomLink | null };

function dialogReducer(state: DialogState, action: DialogAction): DialogState {
    switch (action.type) {
        case 'SET_FIELD': return { ...state, [action.field]: action.value };
        case 'SET_DELETE_CONFIRM': return { ...state, showDeleteConfirm: action.value };
        case 'SET_ICON_POPOVER': return { ...state, iconPopoverOpen: action.value };
        case 'RESET':
            return {
                name: action.editingLink?.name ?? "",
                url: action.editingLink?.url ?? "",
                selectedIcon: action.editingLink?.icon ?? "Link",
...
export default function CustomLinkDialog({
    ...
    const [{ name, url, selectedIcon, showDeleteConfirm, iconPopoverOpen }, dispatch] = useReducer(
        dialogReducer,
        { name: "", url: "", selectedIcon: "Link", showDeleteConfirm: false, iconPopoverOpen: false }
    );

    // Synchroniser les champs avec le lien en cours d'édition
    useEffect(() => {
        dispatch({ type: 'RESET', editingLink });
    }, [open, editingLink]);
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-md">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="space-y-2">
                        <Label>Icône</Label>
                        <Popover open={iconPopoverOpen} onOpenChange={(v) => dispatch({ type: "SET_ICON_POPOVER", value: v })}>
                                ...
                                <div className="grid grid-cols-7 gap-1">
                                    {AVAILABLE_ICONS.map(({ name: iconName, icon: IconComponent }) => (
                                        <button
                                            ...
                                            type="button"
                                            onClick={() => {
                                                dispatch({ type: "SET_FIELD", field: "selectedIcon", value: iconName });
                                                dispatch({ type: "SET_ICON_POPOVER", value: false });
                                            }}
                                            className={`
