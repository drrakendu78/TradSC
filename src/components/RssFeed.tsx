        ...
        <div className="flex flex-col gap-6 p-6">
            ...
            <div className="flex flex-col gap-6">
                {items.map((item, i) => (
                    <article 
                        key={i} 
                        className="bg-card p-4 rounded-lg shadow hover:bg-accent/10 transition-colors cursor-pointer border border-border"
                        onClick={(e) => handleClick(item, e)}
                    >
                        <div className="block space-y-2">
                            <h3 className="font-semibold text-lg hover:text-primary transition-colors group flex items-center">
                                {getText(item.title)}
                                <svg 
                            ...
                            <time className="text-xs text-muted-foreground">{new Date(item.updated).toLocaleString()}</time>
                            <div 
                                className="mt-2 whitespace-pre-line prose prose-sm dark:prose-invert max-w-none"
